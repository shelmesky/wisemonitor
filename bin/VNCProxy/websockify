...
"""
    ...
    def do_proxy(self, target, attached_object):
        """
        Proxy client WebSocket to normal target socket.
        """
        cqueue = []     # 保存发送到WebSocket客户端的数据的缓存
        c_pend = 0 
        tqueue = []     # 保存发送到XenServer的数据的缓存
        
        # 需要监视可读事件的socket: [WebSockets客户的连接, 到XenServer的连接]
        rlist = [attached_object.client, target]

        while True:
            # 每次循环都将可写的socket列表置空
            wlist = []

            # 如果tqueue有数据，则将到XenServer的连接加入到可写socket列表
            if tqueue: wlist.append(target)
            # 如果cqueue有数据，则WebSockets的连接加入到可写socket列表
            if cqueue or c_pend: wlist.append(attached_object.client)
            
            # 监视读写的socket列表
            ins, outs, excepts = select(rlist, wlist, [], 1)
            if excepts: raise Exception("Socket exception")

            # 如果WebSockets连接可写(前面已经根据cqueue将连接加入到wlist)
            # 将cqueue中的数据，用WebSockets协议发送给WebSockets客户端
            if attached_object.client in outs:
                # Send queued target data to the client
                c_pend = self.send_frames(cqueue, attached_object)

                cqueue = []

            # 如果WebSockets连接可读
            if attached_object.client in ins:
                # Receive client data, decode it, and queue for target
                # 从WebSockts连接接收数据，从WebSockets解码到TCP协议
                # 并将解码后的数据，加入到需要发送到XenServer的数据的缓冲中
                bufs, closed = self.recv_frames(attached_object)
                tqueue.extend(bufs)

                if closed:
                    # TODO: What about blocking on client socket?
                    self.vmsg("%s:%s: Client closed connection" %(
                        attached_object.client_address[0],
                        attached_object.client_address[1]))
                    
                    if attached_object.rec:
                        # 关闭到Recoreder Server的连接
                        self.msg("Close recorder server connection when client closed")
                        import os
            ...
            if target in outs:
                ...
                # 发送tqueue中的数据到XenServer
                dat = tqueue.pop(0)
                # TODO: 增加data的translate，再发送到XenServer
            ...
            if target in ins:
                ...
                # 从XenServer接收数据，用WebSockets协议编码
                # 并将数据加入到，发送到WebSockets客户端的数据的缓冲中
                ...
                if len(buf) == 0:
                    ...
                    if attached_object.rec:
                        # 关闭到Recoreder Server的连接
                        self.msg("Close recorder server connection when target closed")
                        import os
                        os.close(attached_object.rec.fileno())
                        attached_object.rec = None
                ...
                # TODO: 增加data的translate，再存入到cqueue
                cqueue.append(buf)
                self.traffic("{")
