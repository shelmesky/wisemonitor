...
if __name__ == '__main__':
    # make a simple test
    def tunnel_listen(session, location, port):
    ...
    for vm in vms:
        ...
        if not record['is_a_template'] and not record['is_control_domain']:
            ...
            port = port_start
            jobs.append(gevent.spawn(tunnel_listen, session_id, console_location, port))
            print "Listening on TCP: <%d> for VM: <%s>" % (port, record['name_label'])
            port_start += 1
    
    print "You can use any VNC Client to connect to this ports."
